...
def preprocess_example(
    ...
    # Alternative implementation for different template format (as suggested in README)
    if "think" in PROMPT_TEMPLATE:
        # Using the alternative template format with <think> tags
        prefix = [
...
def create_training_episodes(
    ...
    Args:
        ...
        remove_std_normalization: If True, removes standard deviation normalization from GRPO
    ...
    for sample, group_indices in zip(samples, groups):
        ...
        # GRPO: subtract the group mean reward and optionally divide by std deviation
        if remove_std_normalization:
            # Experiment: Remove std normalization (as suggested in README)
            advantages = rewards - rewards.mean()
        else:
            # Original GRPO: divide by std deviation
...
def compute_pg_loss(
    ...
    Args:
        ...
        remove_length_normalization: If True, removes the length normalization (as suggested in README)
    ...
    # GRPO: Optionally divide the loss by the length of the responses
    if remove_length_normalization:
        # Experiment: Remove length normalization (as suggested in README)
        loss = (policy_loss + KL_COEFFICIENT * kl_penalty).sum()
    else:
        # Original GRPO: divide by the length of responses
...
def main(args):
    ...
    # Alternative prompt template (as suggested in README for CohenQu model compatibility)
    if args.use_alternative_prompt:
    ...
    # Fixed EOS token handling
    if tokenizer.eos_token_id is not None:
        EOS_TOKEN_ID = tokenizer.eos_token_id
        EOS_TOKEN = tokenizer.convert_ids_to_tokens(EOS_TOKEN_ID)
    else:
        # Fallback if no EOS token is defined
    ...
    for iteration in trange(begin_iter, NUM_ITERATIONS):
        ...
        episodes, episodes_stats = create_training_episodes(
            ...
            remove_std_normalization=args.remove_std_normalization,
        )
        ...
        for i in trange(
            ...
            loss, loss_metrics = compute_pg_loss(
                ...
                remove_length_normalization=args.remove_length_normalization,
            )
...
if __name__ == "__main__":
    ...
    arg_parser.add_argument("--seed", type=int, default=42, help="A number that initializes our random number generator")
    ...
    # New arguments for GRPO experiments (as suggested in README)
    arg_parser.add_argument(
        "--remove_length_normalization",
        action="store_true",
        help="Remove length normalization from GRPO loss computation",
    )
    arg_parser.add_argument(
        "--remove_std_normalization", 
        action="store_true",
        help="Remove standard deviation normalization from advantage computation",
    )
    arg_parser.add_argument(
        "--use_alternative_prompt",
        action="store_true", 
        help="Use alternative prompt template compatible with CohenQu model",
    )
