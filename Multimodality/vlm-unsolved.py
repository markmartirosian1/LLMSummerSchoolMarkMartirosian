...
- Added WSD (Warmup-Stable-Decay) scheduler: 10% warmup, 80% stable, 10% decay
"""
...
class LinearNormalization(nn.Module):
    """Applies z-score normalization across the last dimension."""
...
class TrainingConfig:
    ...
    # WSD Scheduler configs
    warmup_ratio: float = 0.1    # 10% warmup
    stable_ratio: float = 0.8    # 80% stable
    decay_ratio: float = 0.1     # 10% decay
    min_lr_ratio: float = 0.01   # Final LR = 1% of max LR
...
class WSDScheduler:
    ...
    def __init__(
        ...
        console.print(f"[bold cyan]WSD Scheduler Configuration:[/bold cyan]")
        console.print(f"  Total steps: {total_steps}")
        console.print(f"  Warmup steps: {self.warmup_steps} ({warmup_ratio:.1%})")
        console.print(f"  Stable steps: {self.stable_steps} ({stable_ratio:.1%})")
        console.print(f"  Decay steps: {self.decay_steps} ({decay_ratio:.1%})")
        console.print(f"  Max LR: {max_lr:.2e}, Min LR: {self.min_lr:.2e}")
    ...
    def get_lr(self) -> float:
        if self.current_step < self.warmup_steps:
            ...
            lr = self.min_lr + (self.max_lr - self.min_lr) * 0.5 * (
                1 + math.cos(math.pi * decay_step / self.decay_steps)
            )
...
class VisionLanguageConnector(nn.Module):
    ...
    def __init__(
        ...
        layers.append(nn.Linear(in_size, language_hidden_size))
        self.layers = nn.Sequential(*layers)

    def forward(self, vision_features: torch.Tensor) -> torch.Tensor:
        out = self.layers(vision_features)
        out = LinearNormalization()(out)
        return out
...
def train_model(config: TrainingConfig):
    ...
    config_table.add_row("Scheduler", "WSD (10-80-10)")
    ...
    # Setup WSD scheduler instead of linear scheduler
    ...
    for epoch in range(config.num_epochs):
        ...
        with Progress(
            ...
            for step, batch in enumerate(train_loader):
                ...
                optimizer.step()
                current_lr = scheduler.step()  # WSD scheduler step
                optimizer.zero_grad()
                global_step += 1
